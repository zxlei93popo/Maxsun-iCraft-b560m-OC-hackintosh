...
```
![download component](./APPLE/Diskpartition/5.png)
```diff
...
```
![choose installation disk](./APPLE/Diskpartition/3.png)
```
      9.Partition(分区) --- create ESP(EFI)/MSR partition(建立ESP/MSR分区)
```
![create ESP(EFI) and MSR(Recovery) volume](./APPLE/Diskpartition/4.png)
```diff
!     You can change the size from 350MB to 500MB as you like
      10.Change ESP(EFI) volume size
```
![change ESP(EFI) volume size](./APPLE/Diskpartition/6.png)
```
      11.Press 'OK'
```
...
```
![create new partition](./APPLE/Diskpartition/7.png)
```diff
...
+     Recommand cluster size is 4096 bytes
      7.Press 'Format'*(格式化)
```
![format partition](./APPLE/Diskpartition/8.png)  
* ```MacOS partition```
```diff
-     Don't format volume
...
```
  ...
  * ```Mount image```  
  ```  
        ...
        4. Press mount(加载)
  ```  
![load image](./APPLE/Diskpartition/1.png)
  ```
        5.Choose start(启动)
        6.Write harddisk image(写入硬盘映像)
  ```
![write image](./APPLE/Diskpartition/2.png)
  ```diff
...
```
![format disk for Mac](./APPLE/Diskpartition/9.png)
```diff
...
!     You MUST BACKUP your DATA BEFORE the next step,your flash disk WILL BE FORMATTED
      ...
      8.Press 'Yes'
```
![choose image](./APPLE/Diskpartition/10.png)
```diff
      9.Press 'OK'
...
![clover boot](./APPLE/Diskpartition/11.png)
```
...
```
![write boot file](./APPLE/Diskpartition/12.png)
...
![choose platform-id](./APPLE/Patch/1.png)
```
...
```
![__find directory__](./APPLE/Patch/2.png)
```
...
```
![custom USB port](./APPLE/Patch/4.png)
* ```example```
